import React from 'react';



class LuckyMondayTerms extends React.Component {
    constructor(props) {
        super(props);
    }
    render() {
        return(
            <div className="terms_conditions_wrapp">

                <div className="terms_conditions">
                    <div className="terms_conditions_header">Significant Offer Terms and Conditions</div>
                    <ul className="terms_conditions_list circle full_offer">
                        <li>Promotional periods run from 00:00 GMT until 23:59 GMT on 7th, 14th, 21st and 28th January 2019. </li>
                        <li>You must opt in to be eligible for this promotion and you can opt in at any time during January. </li>
                        <li>To earn a Prize Draw ticket you must wager $40 or more in the Casino during a promotional period. Only stakes placed on slot games will count towards the promotion. You can receive a maximum of one Prize Draw ticket per promotional period.</li>
                        <li>Each Prize Draw will take place, and winners will be credited, within 48 hours of the promotional period ending. All prizes will be credited as Free Spins and you will have seven days to accept. To accept your Free Spins you must log in to your Casino account, open a game and click Accept.</li>
                        <li>Any unused Free Spins will be removed after seven days of accepting along with any winnings accrued. Free Spins can only be played on the following slot games: Age of the Gods, Frankie Dettori's Magic 7 Jackpot, Age of the Gods: Rulers of Olympus, Frankie Dettori: Sporting Legends, Age of the Gods: Furious 4, Age of the Gods: King Of Olympus and Zhao Cai Jin Bao.</li>
                    </ul>
                </div>

                <div className="terms_conditions">
                    <div className="terms_conditions_header">Full Offer Terms and Conditions</div>
                    <ul className="terms_conditions_list full_offer">
                        <li>Promotional periods run from 00:00 GMT until 23:59 GMT on 7th, 14th, 21st and 28th January 2019. </li>
                        <li>You must opt in to be eligible for this promotion and you can opt in at any time during January. </li>
                        <li>To earn a Prize Draw ticket you must wager $40 or more in the Casino during a promotional period. Only stakes placed on slot games will count towards the promotion.</li>
                        <li>You can receive a maximum of one Prize Draw ticket per promotional period.</li>
                        <li>There is a total of $2,000 in cash available in each Prize Draw. For a full breakdown of prizes available, see the above table.</li>
                        <li>Each Prize Draw will take place, and winners will be credited, within 48 hours of the promotional period ending.</li>
                        <li>All prizes will be credited as Free Spins and you will have seven days to accept. To accept you must log in to your Casino account, open a game and click Accept. </li>
                        <li>Once accepted you will have seven days to use the Free Spins. Any unused Free Spins will be removed after seven days of accepting along with any winnings accrued.</li>
                        <li>Free Spins can only be played on the following slot games: Age of the Gods, Frankie Dettori's Magic 7 Jackpot, Age of the Gods: Rulers of Olympus, Frankie Dettori: Sporting Legends, Age of the Gods: Furious 4, Age of the Gods: King Of Olympus and Zhao Cai Jin Bao.</li>
                        <li>Any prize/bonus amounts offered will be calculated using the current currency multiplier table as determined at the sole discretion of bet365.</li>
                        <li>In an attempt to combat fraudulent activity, bet365 has introduced methods to detect and prevent abuse of this offer. bet365 may ask any customer to provide sufficient documentation and/or verification steps for it, acting reasonably, to be satisfied as to the customer's identity prior to crediting any offer, bonus funds and/or winnings to their account.</li>
                        <li>Where any term of the offer is breached or there is evidence of a series of wagers placed by a customer or group of customers, which due to a deposit bonus, enhanced payments or any other promotional offer results in guaranteed customer profits irrespective of the outcome, whether individually or as part of a group, bet365 may reclaim the bonus or enhanced payment element of such offer, void any wager funded through the offer and/or remove any winnings from such wager. In addition, where there is evidence of such activity, bet365 may levy an administration charge on the customer up to the value of the bonus or enhanced payment towards the administrative costs incurred in identifying and taking action against that activity.</li>
                        <li>All customer offers are limited to one per person. If bet365 has reasonable grounds to suspect that the bonus or offer is being claimed by or for the benefit of the same person more than once or by a group of people then it may withdraw the availability of any offer or all offers to that customer or group of customers and/or void any wager funded by the bonus and remove any winnings from such wager.</li>
                        <li>bet365 may, at any time, make minor amendments to this promotion to correct typos or to improve on clarity or customer experience and may cancel this promotion for legal or regulatory reasons.</li>
                        <li>Employees, officers and directors of bet365, its promotional or other agencies, licensees and licensors, service providers and any other associated or affiliated companies shall not be eligible for the promotion. The same terms shall apply to the direct families of such persons.</li>
                    </ul>
                </div>
            </div>
        )

    }
}

export default LuckyMondayTerms;